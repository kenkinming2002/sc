...
static inline int item_key_compare(int *item1, int *item2) { return *item1 - *item2; }
...
int main()
{
  ...
  for(int i=0; i<100; ++i)
    assert(item_avl_tree_lookup(&items, &(int){2 * i}));

  // 3: Looking up non-existent node
  for(int i=0; i<100; ++i)
    assert(!item_avl_tree_lookup(&items, &(int){2 * i + 1}));

  // 3: Removing existent node
  for(int i=21; i<47; ++i)
  {
    struct item *item = item_avl_tree_remove(&items, &(int){2 * i});
    assert(item);
    free(item);
  ...
  for(int i=53; i<87; ++i)
  {
    struct item *item = item_avl_tree_remove(&items, &(int){2 * i + 1});
    assert(!item);
    item_avl_tree_check(&items);
  }
